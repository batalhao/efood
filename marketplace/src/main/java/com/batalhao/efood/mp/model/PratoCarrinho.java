...
public class PratoCarrinho {
  ...
  public static Uni<Long> save(PgPool client, String cliente, Long prato) {
    String query = "INSERT INTO prato_cliente (cliente, prato) VALUES ($1, $2) RETURNING (cliente)";
    return client.preparedQuery(query).execute(Tuple.of(cliente, prato))
  ...
  public static Uni<List<PratoCarrinho>> findCarrinho(PgPool client, String cliente) {
    String query = "select * from prato_cliente where cliente = $1 ";
    return client.preparedQuery(query).execute(Tuple.of(cliente)).map(pgRowSet -> {
  ...
  public static Uni<Boolean> delete(PgPool client, String cliente) {
    String query = "DELETE FROM prato_cliente WHERE cliente = $1";
    return client.preparedQuery(query).execute(Tuple.of(cliente))
